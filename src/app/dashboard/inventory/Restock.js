...
function Restock({ name, productId , reload}) {
  ...
  const updatedProduct = {
    ...response.data,
    items_remaining: response.data.items_remaining + newItems,
    category: category ? category.id : null // Use the ID of the matching category, or null if no matching category was found
  };
    ...
    <div className="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl m-3 p-5">
      <form onSubmit={handleSubmit}>
        ...
        <div className="mt-4">
          ...
          <input type="number" value={newItems} onChange={(e) => setNewItems(Number(e.target.value))} className="mt-1 block w-full rounded-md border-gray-300 border-2 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" />
        </div>
        <div className="mt-4">
